...
class Node:
    def __init__(self, name: str, children: List['Node'], status: Literal["scheduled", "running", "failed", "succeeded"], num_descendants: int, node_type: Literal["job_group", "job"]):
        self.name = name
...
# A helper function to render the Node tree as an HTML unordered list.
def render_tree(node: Node) -> str:
    ...
    html = f'<li><span style="color: {color}; font-weight: bold;">{node.name} ({node.status})'
...
def index():
    ...
    # Use your HTML template with Bootstrap
