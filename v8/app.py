...
def get_log():
    ...
    filename = request.args.get("filename", "log.txt")  # Default to log.txt if not specified
    
    # Handle HEAD requests to check if file exists
    ...
    # For demonstration purposes, create example files if they don't exist
    if not os.path.exists(log_path):
        try:
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            
            # Create different content for different file types
            if filename == "log.txt":
