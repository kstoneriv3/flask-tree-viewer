...
# Helper function to recursively generate HTML with data attributes
def render_tree(node: Node) -> str:
    ...
    # Each <li> has data attributes and an onclick that invokes a JS function.
    html = (
        f'<li data-name="{node.name}" '
        f'data-status="{node.status}" '
        f'data-numdesc="{node.num_descendants}" '
        f'data-nodetype="{node.node_type}" '
        f'onclick="selectNode(this)" style="cursor: pointer;">'
    )
...
# Example Node tree. In your case, this tree is obtained after parsing the DAG.
example_tree = Node(
    name="Root Group",
    status="running",
    num_descendants=4,
